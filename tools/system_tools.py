...
def service_manage(action: str, name: str = "", lines: int = 30) -> ToolResult:
    try:
        if action == "status":
            ...
            result = subprocess.run(["systemctl", "status", name, "--no-pager"], capture_output=True, text=True, timeout=30)
            ...
            result = subprocess.run(["systemctl", "restart", name], capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                status_result = subprocess.run(["systemctl", "is-active", name], capture_output=True, text=True, timeout=10)
            ...
            for svc in sorted(AGENT_SERVICES):
                check = subprocess.run(["systemctl", "list-unit-files", f"{svc}.service"], capture_output=True, text=True, timeout=10)
                if f"{svc}.service" not in check.stdout:
                    continue
                status = subprocess.run(["systemctl", "is-active", svc], capture_output=True, text=True, timeout=10)
                enable = subprocess.run(["systemctl", "is-enabled", svc], capture_output=True, text=True, timeout=10)
            ...
            if not name:
                return ToolResult.fail("请指定服务名称")
            result = subprocess.run(["journalctl", "-u", name, "-n", str(lines), "--no-pager"], capture_output=True, text=True, timeout=30)
            if result.returncode != 0:
...
def network_diag(action: str, target: str = "8.8.8.8", count: int = 3) -> ToolResult:
    try:
        if action == "interfaces":
            result = subprocess.run(["ip", "-j", "addr"], capture_output=True, text=True, timeout=30)
            if result.returncode == 0 and result.stdout.strip():
                ...
                try:
                    ...
                    pass
            result = subprocess.run(["ip", "addr"], capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                return ToolResult.ok(f"网络接口信息:\n{result.stdout.strip()}")
            return ToolResult.fail("获取网络接口信息失败")
        elif action == "ping":
            result = subprocess.run(["ping", "-c", str(count), "-W", "3", target], capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
            ...
            result = subprocess.run(["ss", "-tlnp"], capture_output=True, text=True, timeout=30)
            if result.returncode != 0:
            ...
            result = subprocess.run(["nslookup", target], capture_output=True, text=True, timeout=30)
            if result.returncode == 0 and result.stdout.strip():
                return ToolResult.ok(f"DNS解析 {target}:\n{result.stdout.strip()}")
            result = subprocess.run(["dig", "+short", target], capture_output=True, text=True, timeout=30)
            if result.returncode == 0 and result.stdout.strip():
                return ToolResult.ok(f"DNS解析 {target}:\n{result.stdout.strip()}")
...
@register_tool(
    ...
    schema={
        ...
        "properties": {
            ...
            "path": {"type": "string", "description": "项目路径", "default": "/home/orangepi/ai-agent"},
            "lines": {"type": "integer", "description": "日志行数", "default": 50},
            "service": {"type": "string", "description": "服务名称", "default": "nahida"},
        },
        "required": ["action"],
    },
    permission=ToolPermission.READ_ONLY,
...
def dev_assist(action: str, path: str = "/home/orangepi/ai-agent", lines: int = 50, service: str = "nahida") -> ToolResult:
    try:
        if action == "git_status":
            ...
            status_result = subprocess.run(["git", "status", "--short"], capture_output=True, text=True, timeout=30, cwd=path)
            log_result = subprocess.run(["git", "log", "--oneline", "-5"], capture_output=True, text=True, timeout=30, cwd=path)
            parts = []
            ...
            check_result = subprocess.run(["pip", "check"], capture_output=True, text=True, timeout=30)
            key_packages = ["openai", "aiosqlite", "loguru", "qq-botpy", "aiohttp", "fastapi", "uvicorn", "pydantic", "httpx"]
            pip_list = subprocess.run(["pip", "list", "--format=columns"], capture_output=True, text=True, timeout=30)
            parts = []
            ...
            if os.path.isdir(log_dir):
                log_files = sorted([f for f in os.listdir(log_dir) if f.endswith(".log")], key=lambda f: os.path.getmtime(os.path.join(log_dir, f)), reverse=True)
                if not log_files:
                    log_result = subprocess.run(["journalctl", "-u", service, "-n", str(lines), "--no-pager"], capture_output=True, text=True, timeout=30)
            ...
            result = subprocess.run(["find", path, "-maxdepth", "2", "-not", "-path", "*/__pycache__/*", "-not", "-path", "*/.git/*", "-not", "-name", "*.pyc"], capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
