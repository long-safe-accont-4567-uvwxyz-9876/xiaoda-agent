...
class HuggingFaceRepository:
    ...
    async def inspect(
        ...
        try:
            files, config = await loop.run_in_executor(
                None, self._collect, repository, revision, token
            )
    ...
    def _resolve_small_files(
        ...
        def work(path: str) -> tuple[str, str | None, str | None]:
            try:
        ...
        with ThreadPoolExecutor(max_workers=4) as pool:
            for path, digest, local in pool.map(work, pending):
                if digest:
                    sha_map[path] = digest
                if path == "config.json" and local and config is None:
                    try:
                        with open(local, "rb") as handle:
