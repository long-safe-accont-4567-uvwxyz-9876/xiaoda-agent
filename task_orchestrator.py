...
请只返回Agent名称（多个用逗号分隔）:"""
    ...
    async def route(self, state: TaskState) -> dict:
        """路由入口：简化为只处理 @mention 和并行任务，其他都路由到 xiaoda。

        设计原则：
        1. @mention 路由：用户明确指定子代理时，直接路由
        2. 并行任务：用户明确要求并行处理时，路由到多个子代理
        3. 其他情况：都路由到 xiaoda，由主LLM通过 delegate_task 工具决定是否需要子代理
        """
        ...
        # 2. 并行任务检测（用户明确要求并行处理，如"分别问小莉和小狼..."）
        ...
        # 3. 其他情况：都路由到 xiaoda，由主LLM通过 delegate_task 工具决定
        # 这是核心设计：主LLM会根据对话上下文和用户意图，自主决定是否需要调用子代理
        return {"route_targets": ["xiaoda"], "route_target": "xiaoda", "route_plan": ["xiaoda"]}
