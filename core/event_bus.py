...
class AgentEventBus:
    ...
    async def emit(self, event: AgentEvent) -> None:
        ...
        deliver 保持同步 await：emit 的"投递即送达"契约不变（子代理/工具事件
        需要可靠送达）。慢投递通道（WebSocket/QQ）在传输层已改为非阻塞
        （ws_hub 有界队列 / qq 后台 _spawn），因此此处 await 不会阻塞关键路径。
        """
        user = _current_user.get()
        if user is None:
            return
        await _deliver(user, event)
...
def gen_task_id(agent: str, input_hint: str = "") -> str:
    ...
    if input_hint:
        hint_hash = hashlib.md5(input_hint.encode(), usedforsecurity=False).hexdigest()[:4]
