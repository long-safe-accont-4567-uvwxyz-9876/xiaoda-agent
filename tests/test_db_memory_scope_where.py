...
def test_scope_where_without_is_raw():
    ...
    assert where == (
        " AND user_id = ? AND agent_id = ? AND COALESCE(session_id, '') NOT LIKE ?"
        " AND COALESCE(session_id, '') != 'archived'"
    )
    assert params == ["alice", "xiaoli", "qq_group:%"]


def test_scope_where_with_is_raw():
    where, params = _scope_where(Scope(), is_raw=0)
    assert where.endswith(" AND is_raw = ?")
    assert params == ["default", "xiaoda", "qq_group:%", 0]
...
def test_scope_where_with_table_prefix_no_archived():
    ...
    assert where == (
        ...
        " AND COALESCE(em.session_id, '') NOT LIKE ? AND em.is_raw = ?"
    )
    assert params == ["a", "b", "qq_group:%", 1]
...
def test_private_scope_excludes_group_sessions_but_shares_private_sessions():
    ...
    assert "COALESCE(session_id, '') NOT LIKE ?" in where
