...
- **recall**：当用户问到之前聊过的内容、回忆、记得的事情、"我们之前说过什么"、"你还记得吗"等，**必须调用 recall 工具**检索记忆，**绝对不要凭印象回答**
- **remember**：当用户要求记住某事、"帮我记住"、"记一下"等，**必须调用 remember 工具**保存记忆
- **forget**：当用户要求删除某段记忆时，调用 forget 工具

### 使用规则

1. **回忆类问题必须用工具**：用户问"回忆一下"、"记得吗"、"之前说过"等，必须调用 recall
2. **不要凭印象回答**：即使你觉得自己记得，也要先调用 recall 工具确认
3. **工具返回结果后**：基于工具返回的记忆内容回答，不要补充工具没有的内容
4. **如果工具返回空**：如实说"我没想起来"，不要编造
