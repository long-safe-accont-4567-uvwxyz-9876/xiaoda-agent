...
async def _broadcast_kg_change(action: str, target: str, name: str) -> None:
    ...
    try:
        ...
        logger.warning("insight.kg.broadcast_failed action={} target={}: {}", action, target, e)
    except Exception:
        logger.exception("insight._broadcast_kg_change.unexpected_error")
# ── 情绪 ─────────────────────────────────────────────────────────
    ...
    async def _run() -> None:
        try:
            ...
            try:
                ...
                logger.exception("insight.portrait_broadcast_failed unexpected_error")

        except Exception:
            logger.exception("insight._run.unexpected_error")
...
async def today(request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.today.unexpected_error")
...
async def list_memories(request: Request,
    ...
    if q.strip() and core.memory:
        try:
            # 分页语义与 DB 分支对齐：向量检索按相关性排序，取 k = 跳过量 +
            # 本页条数后切本页窗口（同名接口不再有两种分页语义）
            results = await core.memory.retrieve_memories(
                q.strip(), k=page * limit + limit)
            window = results[page * limit:(page + 1) * limit]
            return Envelope(data=[
            ...
            logger.exception("insight.list_memories.unexpected_error")
    rows = await core.db.fetch_all(
...
async def delete_memory(memory_id: int, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.delete_memory.unexpected_error")
        raise HTTPException(500, "操作失败") from None
...
async def knowledge_graph(request: Request,
    ...
    depth=1 只返回第一跳。每层独立 GRAPH_MAX_EDGES_PER_HOP 上限
    （confidence 降序截断，被挤掉的边所指向的实体不再外扩），
    防止热门实体（1200+ 关系）撑爆渲染且不再掩盖深度差异。

    空实体（全图模式）：自动选关系数最高的实体作为根再做同款 BFS ——
    此前该分支固定返回最近 80 条关系、完全忽略 depth，
    导致前端调深度无任何变化（2026-08-25 冒烟发现）。
    """
    core = request.app.state.core
    kdb = core.db.knowledge
    # 分层可视化参数：每层最多 30 个新实体（按关系 confidence 挑最重要的），
    # 边上限随之收敛。depth=1 = 根实体 + 一跳邻居，逐层外扩一圈一圈清晰。
    ...
    if entity.strip():
        ...
        # 全图模式：以"度数最高"的实体为根 —— 记忆树的树干顶
        cur0 = await kdb._conn.execute(
            "SELECT name FROM ("
            "  SELECT from_entity AS name FROM knowledge_relations"
            "  UNION ALL SELECT to_entity FROM knowledge_relations"
    ...
    # 一跳就吃满名额，depth=1 与 depth=6 返回完全一致（约束失效）。
    seen_rel: set[tuple[str, str, str]] = set()
    visited: set[str] = {seed}
    prev_kept: set[str] = {seed}  # 上一层最终保留节点（逐层更新为扩散中心）
    frontier = [seed]
    edges: list[dict] = []
    ...
    for _ in range(depth):
        ...
        if len(next_frontier) > GRAPH_MAX_NODES_PER_HOP:
            ...
            # prev_kept 首层=根实体；一跳时未入选邻居的边因另一端不在
    ...
    # 节点只来自保留边的端点——未入选的邻居不返回（否则图上挂一堆孤立点）
...
async def update_relation(relation_id: str, body: dict, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.update_relation.unexpected_error")
        raise HTTPException(500, "操作失败") from None
    if not n:
...
async def create_note(body: dict, request: Request) -> Any:
    ...
    note_id = await core.db.notebook.insert_notebook(
        ...
        importance=_safe_float(body.get("importance", 0.5), 0.5))
    await core.db.commit()
...
async def update_note(note_id: int, body: dict, request: Request) -> Any:
    ...
    if "importance" in body and body["importance"] is not None:
        ...
        params.append(_safe_float(body["importance"], 0.5))
    if not sets:
...
async def list_instincts(request: Request, limit: int = Query(default=50, le=200)) -> Any:
    ...
    try:
        ...
        logger.exception("insight.list_instincts.unexpected_error")
    return Envelope(data=rows)
...
async def create_memory(body: dict, request: Request) -> Any:
    ...
    ts = _safe_float(body.get("timestamp", time.time()), default=time.time())
    mid = await core.db.memory.insert_episodic_memory(
        summary=summary, importance=importance,
        emotion_label=emotion_label)
    # 补录语义：允许指定记忆发生时间（repo 层固定写 now，这里显式回填）
    await core.db.execute(
        "UPDATE episodic_memories SET timestamp=? WHERE id=?", (ts, mid))
    # 写入向量索引（失败时记录警告，不静默吞掉）
    try:
        if core.memory:
            await core.memory.vec.upsert(mid, summary)
    except (OSError, KeyError, ValueError, RuntimeError, TypeError) as e:
        logger.warning("insight.create_memory.vec_failed mid={}: {}", mid, e)
    except Exception:
        logger.exception("insight.create_memory.unexpected_error")
    await core.db.commit()
...
async def update_memory(memory_id: int, body: dict, request: Request) -> Any:
    ...
    if "summary" in body and body["summary"]:
        try:
            ...
            logger.exception("insight.update_memory.unexpected_error")
    await core.db.commit()
...
async def create_learning(body: dict, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.create_learning.unexpected_error")
        raise HTTPException(500, "操作失败") from None
...
async def update_learning(learning_id: int, body: dict, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.update_learning.unexpected_error")
        raise HTTPException(500, "操作失败") from None
    if not n:
...
async def delete_learning(learning_id: int, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.delete_learning.unexpected_error")
        raise HTTPException(500, "操作失败") from None
    if not n:
        raise HTTPException(404, f"学习记录 {learning_id} 不存在")
    try:
...
async def create_instinct(body: dict, request: Request) -> Any:
    ...
    if not content:
        raise HTTPException(400, "content 不能为空")
    confidence = _safe_float(body.get("confidence", 0.5), 0.5)
    now = time.time()
    try:
        ...
        logger.exception("insight.create_instinct.unexpected_error")
        raise HTTPException(500, "操作失败") from None
...
async def update_instinct(instinct_id: int, body: dict, request: Request) -> Any:
    ...
    if "confidence" in body and body["confidence"] is not None:
        ...
        params.append(_safe_float(body["confidence"], 0.5))
    if not sets:
        raise HTTPException(400, "无可更新字段")
    ...
    try:
        ...
        logger.exception("insight.update_instinct.unexpected_error")
        raise HTTPException(500, "操作失败") from None
    if not n:
...
async def delete_instinct(instinct_id: int, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.delete_instinct.unexpected_error")
        raise HTTPException(500, "操作失败") from None
    if not n:
        raise HTTPException(404, f"本能 {instinct_id} 不存在")
    try:
...
async def create_relation(body: dict, request: Request) -> Any:
    ...
    try:
        ...
        logger.exception("insight.create_relation.unexpected_error")
        raise HTTPException(500, "操作失败") from None
...
async def get_xp(request: Request, user_id: str = Depends(get_current_user)) -> Any:
    ...
    try:
        ...
        logger.warning("insight.xp.get_failed user={}: {}", user_id, e)
        raise HTTPException(500, "获取 XP 状态失败") from None
    except Exception:
        logger.exception("insight.unknown.unexpected_error")
        raise HTTPException(500, "获取 XP 状态失败") from None
...
async def get_xp_levels(request: Request) -> Any:
    ...
    try:
        ...
        logger.warning("insight.xp.levels_failed: {}", e)
        raise HTTPException(500, "获取 XP 等级配置失败") from None
    except Exception:
        logger.exception("insight.get_xp_levels.unexpected_error")
        raise HTTPException(500, "获取 XP 等级配置失败") from None
