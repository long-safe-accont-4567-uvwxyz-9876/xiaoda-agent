...
非官方声明：本项目为个人学习用途的非官方二次创作，与原作权利方不存在任何隶属、授权或赞助关系。

知识产权：本项目中涉及的角色名称、形象、设定、语音、表情素材等知识产权均归其原始版权方所有。上述素材以非官方、粉丝向目的使用，并依据原始版权方的公开社区准则和合理使用原则进行引用。

代码：本项目的全部代码、架构设计、文档由开发者独立编写，以 MIT 协议开源。

第三方素材：本项目可能引用或集成第三方开源素材（字体、图标、音频等），其版权和许可条款以各自原始项目为准。

侵权处理：如原始版权方认为本项目存在侵权或不当内容，请联系开发者，开发者将在确认后立即删除相关内容。
...
6. 数据外发：本软件在运行过程中会将用户的输入文本、对话内容、邮件数据等通过互联网传输至第三方大模型 API（如 SiliconFlow、OpenRouter、ModelScope 等）进行推理，用户应在敏感场景（如涉及个人隐私、商业机密、未成年人信息等）自行评估数据外发风险。开发者对第三方服务的隐私政策、数据留存策略、安全合规性不作任何背书或担保。

【四、AI 生成内容风险】

7. 内容风险：本软件输出的文本、代码、图片、表情等内容均由人工智能模型概率性生成，可能存在事实性错误、逻辑偏差、安全漏洞或不当内容。用户在对外引用、转发、商用、发布或用于医疗、法律、金融等高风险决策场景前，必须自行核实并由具备资质的专业人士复核。开发者不对 AI 生成内容的合法性、准确性、可用性、安全性作任何保证，由此产生的后果由用户本人承担。

【五、第三方依赖】

8. 第三方服务免责：本软件集成的第三方模型服务、MCP 服务、插件、表情素材、agently-cli 邮箱能力等组件，其服务条款、计费规则、可用性、政策变更及合规风险均由对应第三方主体负责。开发者对第三方服务的稳定性、连续性、合法性不承担任何责任，因第三方服务中断、变更、合规问题等造成的损失由用户自行承担。"""
