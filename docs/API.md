...
获取当前情感状态。返回值为 `Emotion` 枚举成员之一（共 16 种：HAPPY / EXCITED / LOVE / SHY / SAD / ANGRY / SURPRISED / CONFUSED / THINKING / PLAYFUL / MOVED / NEUTRAL / ANXIOUS / FEAR / CURIOUS / POUT）。

### GET /insight/emotion/history
获取情感变化历史。每条记录的 `emotion` 字段为上述 16 种核心枚举之一，原始中文/变体标签由 `EMOTION_ALIASES` 归并而来。
