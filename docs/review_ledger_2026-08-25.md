...
## 十三、接续冒烟：知识图谱深度修复的验证闭环（修复属并行会话）

并行会话在真实冒烟中发现并修复了 `/insight/knowledge/graph` 空实体模式
忽略 depth 参数的问题（旧代码固定返回最近 80 条关系），重构为统一逐层 BFS
（双分支仅 seed 不同：显式实体 vs 度数最高实体；每层 30 节点/90 边上限）。
本侧完成验证闭环：

- 单测 tests/test_insight_knowledge_graph.py 4 passed；ruff 干净
- 重启后活体实测：
  - 空实体：depth=1→31/32，2→38/42，3→40/46，4→40/46（单调递增后饱和，
    符合图直径有限的预期；修复前 depth=1 与 2 恒等 26/80）
  - 指定实体"7 月 19 日"：depth=1 严格 2 节点/1 边，depth=3 外扩至 39/43
...
### 事故与处置披露：kg 优化的提交归属链（第十五章补二）

时间线：kg 预热+single-flight 实现停笔于工作区 → 本侧 `git commit` 时将并行
会话**已暂存的 dist/前端改动**连带卷入（57e84d7c，消息误导）→ soft-reset 拆分
还原 → 发现并行会话的 95d0422b 批量提交已先行吸收 kg 正文实现 → 最终仅补提
交遗留的单测文件。教训升级：**双会话并存期间，commit 前必须 `git status` 全览
暂存区归属；无路径 `git commit` 在对方有 staged 内容时等于代提交。**

性能结论不变：实体抽取预热前移+single-flight 已在生产运行（16:31 起的重启
即含此变更），channel.kg 的"付费后被扔"模式应随窗口重叠消除——后续观察
channel.kg.error 计数是否归零即可验证。
